#!/usr/bin/env node
/**
 * Smoke test for Test Label Values HTTP API endpoint:
 * - POST /api/tools/horreum_get_test_label_values
 *
 * Usage:
 *   node scripts/smoke-get-test-label-values.mjs [BASE_URL] [TOKEN]
 */
...
async function main() {
  log('INFO', `Horreum MCP Test Label Values Endpoint Smoke Test`);
  log('INFO', `Base URL: ${BASE_URL}`);
  log('INFO', '');

  const results = [];

  // Test 1: Basic request by test_id
  results.push(
    await testEndpoint(
      'horreum_get_test_label_values (by test_id)',
      `${BASE_URL}/api/tools/horreum_get_test_label_values`,
      {
        test_id: 262,
        limit: 5,
        page: 1,
  ...
  // Test 2: By test_name with natural language time
  results.push(
    await testEndpoint(
      'horreum_get_test_label_values (by test_name, last week)',
      `${BASE_URL}/api/tools/horreum_get_test_label_values`,
      {
  ...
  // Test 3: Missing identifier (should fail)
  results.push(
    await testEndpoint(
      'horreum_get_test_label_values (missing id - should fail)',
      `${BASE_URL}/api/tools/horreum_get_test_label_values`,
      {},
      400
    )
