#!/usr/bin/env node
/**
 * Smoke test for Dataset Label Values HTTP API endpoint:
 * - POST /api/tools/horreum_get_dataset_label_values
 *
 * Usage:
 *   node scripts/smoke-get-dataset-label-values.mjs [BASE_URL] [TOKEN]
 */
...
async function main() {
  log('INFO', `Horreum MCP Dataset Label Values Endpoint Smoke Test`);
  ...
  results.push(
    await testEndpoint(
      'horreum_get_dataset_label_values (basic)',
      `${BASE_URL}/api/tools/horreum_get_dataset_label_values`,
      {
        dataset_id: 12345,
      }
    )
  ...
  // Test 2: Missing dataset_id (should fail)
  results.push(
    await testEndpoint(
      'horreum_get_dataset_label_values (missing id - should fail)',
      `${BASE_URL}/api/tools/horreum_get_dataset_label_values`,
      {},
      400
    )
