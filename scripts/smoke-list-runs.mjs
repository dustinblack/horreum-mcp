...
await registerTools(server, {
  ...
  fetchImpl: async (url) => {
    ...
    if (u.pathname.startsWith('/api/run/list/123')) {
      ...
      const now = Date.now();
      const mk = (i) => ({
        access: 'PUBLIC', owner: 'mock', id: i, testid: 123, testname: 'example-test',
        start: now - i * 3600_000, stop: now - i * 3600_000 + 60_000,
        trashed: false, hasMetadata: false, datasets: []
      });
...
const res = await client.callTool({ name: 'list_runs', arguments: { test: 'example-test', from } });
